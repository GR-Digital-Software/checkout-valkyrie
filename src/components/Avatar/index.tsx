...
const Avatar: React.FC<AvatarProps> = ({
    ...
    const backgroundColorClass = bgColor === 'zinc' ? 'bg-zinc-100' : 'bg-primary-500';
    const textColorClass = bgColor === 'zinc' ? 'text-zinc-900' : 'text-white';

    return (
        <div
            className={`flex items-center justify-center rounded-full ${sizeClasses[size]} ${imageUrl ? '' : `${backgroundColorClass} ${textColorClass}`}`}
        >
            {imageUrl ? (
                <Image
                    src={imageUrl}
